...
import {
  ...
  require_prop_types,
  sum
} from "./chunk-I6B2NDPF.js";
import {
  cubehelix,
  cubehelixLong,
  rgb,
  rgbBasis
} from "./chunk-PZMWHMFU.js";
import {
  require_react
} from "./chunk-L4EBCQGQ.js";
import {
  __toESM
...
var _default = (function() {
  ...
  return _createClass2(_default11, [{
    ...
    value: function lookup(color) {
      ...
      var n2 = typeof color === "string" ? colorStr2Int(color) : rgb2Int.apply(void 0, _toConsumableArray2(color));
      if (!n2) return null;
...
var utils = {
  ...
  compute: function(t3, points, _3d) {
    ...
    if (order < 4) {
      let mt2 = mt * mt, t22 = t3 * t3, a3, b2, c5, d2 = 0;
      if (order === 2) {
      ...
      const ret = {
        x: a3 * p2[0].x + b2 * p2[1].x + c5 * p2[2].x + d2 * p2[3].x,
        y: a3 * p2[0].y + b2 * p2[1].y + c5 * p2[2].y + d2 * p2[3].y,
        t: t3
      };
      if (_3d) {
        ret.z = a3 * p2[0].z + b2 * p2[1].z + c5 * p2[2].z + d2 * p2[3].z;
      }
      return ret;
    }
  ...
  roots: function(points, line) {
    ...
    if (order === 2) {
      const a4 = aligned[0].y, b3 = aligned[1].y, c6 = aligned[2].y, d3 = a4 - 2 * b3 + c6;
      if (d3 !== 0) {
        const m1 = -sqrt(b3 * b3 - a4 * c6), m22 = -a4 + b3, v12 = -(m1 + m22) / d3, v2 = -(-m1 + m22) / d3;
        return [v12, v2].filter(reduce2);
      } else if (b3 !== c6 && d3 === 0) {
        return [(2 * b3 - c6) / (2 * b3 - 2 * c6)].filter(reduce2);
      }
      return [];
    }
    const pa = aligned[0].y, pb = aligned[1].y, pc = aligned[2].y, pd = aligned[3].y;
    let d2 = -pa + 3 * pb - 3 * pc + pd, a3 = 3 * pa - 6 * pb + 3 * pc, b2 = -3 * pa + 3 * pb, c5 = pa;
    if (utils.approximately(d2, 0)) {
      if (utils.approximately(a3, 0)) {
        if (utils.approximately(b2, 0)) {
          return [];
        }
        return [-c5 / b2].filter(reduce2);
      }
      const q3 = sqrt(b2 * b2 - 4 * a3 * c5), a22 = 2 * a3;
      return [(q3 - b2) / a22, (-b2 - q3) / a22].filter(reduce2);
    }
    a3 /= d2;
    b2 /= d2;
    c5 /= d2;
    const p2 = (3 * b2 - a3 * a3) / 3, p3 = p2 / 3, q2 = (2 * a3 * a3 * a3 - 9 * a3 * b2 + 27 * c5) / 27, q22 = q2 / 2, discriminant = q22 * q22 + p3 * p3 * p3;
    let u1, v1, x1, x22, x3;
    if (discriminant < 0) {
  ...
  droots: function(p2) {
    if (p2.length === 3) {
      const a3 = p2[0], b2 = p2[1], c5 = p2[2], d2 = a3 - 2 * b2 + c5;
      if (d2 !== 0) {
        const m1 = -sqrt(b2 * b2 - a3 * c5), m22 = -a3 + b2, v1 = -(m1 + m22) / d2, v2 = -(-m1 + m22) / d2;
        return [v1, v2];
      } else if (b2 !== c5 && d2 === 0) {
        return [(2 * b2 - c5) / (2 * (b2 - c5))];
      }
      return [];
    }
    if (p2.length === 2) {
  ...
  inflections: function(points) {
    ...
    const p2 = utils.align(points, { p1: points[0], p2: points.slice(-1)[0] }), a3 = p2[2].x * p2[1].y, b2 = p2[3].x * p2[1].y, c5 = p2[1].x * p2[2].y, d2 = p2[3].x * p2[2].y, v1 = 18 * (-3 * a3 + 2 * b2 + 3 * c5 - d2), v2 = 18 * (3 * a3 - b2 - 3 * c5), v3 = 18 * (c5 - a3);
    ...
    tickFrame: function tickFrame(state) {
      ...
      function paintLinks() {
        ...
        Object.entries(linksPerColor).forEach(function(_ref) {
          var _ref2 = _slicedToArray5(_ref, 2), color = _ref2[0], linksPerWidth = _ref2[1];
      ...
      function paintArrows() {
        ...
        state.graphData.links.filter(getVisibility).forEach(function(link) {
          ...
          var bzLine = link.__controlPoints && _construct(Bezier, [start.x, start.y].concat(_toConsumableArray4(link.__controlPoints), [end.x, end.y]));
      ...
      function paintPhotons() {
        ...
        state.graphData.links.filter(getVisibility).forEach(function(link) {
          ...
          var bzLine = link.__controlPoints ? _construct(Bezier, [start.x, start.y].concat(_toConsumableArray4(link.__controlPoints), [end.x, end.y])) : null;
